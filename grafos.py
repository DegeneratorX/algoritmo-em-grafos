...
class Grafo:
    ...
    def is_star_graph(self):
        ...
        for i in range(self.num_vertices): # Itero sobre os vértices do grafo
            ...
            if len(self.lista_adj[i]) == self.num_vertices-1: 
        ...
        for i in range(self.num_vertices):
            ...
            if i not in vertices_centrais and (len(self.lista_adj[i]) != 1 or self.lista_adj[i][0] not in vertices_centrais):
    ...
    def bfs(self, origem):
        ...
        distances = [float("inf")] * len(self.lista_adj)
        distances[origem] = 0 # Defino a distância para a própria origem como zero.

        # Inicio uma fila de prioridade com origem zero.
        queue = [origem]
        ...
        while queue: # Enquanto ela não estiver vazia
            ...
            # agora.
            for neighbor in self.lista_adj[curr_node]:
