 ...
 * Usando apenas Lista de Adjacências
 */


class Grafo {
public:
    std::vector<std::vector<std::pair<int, float>>> lista_adj;
    ...
    float fluxo_maximo(int origem, int destino) {
        ...
        while (true) {
            ...
            for (const auto& [u, v] : caminho) {
                float capacidade = get_capacidade(u, v) - fluxos[u][v];
...
private:
    ...
    void tratamento_dos_dados(const std::vector<std::string>& linhas, bool tem_peso) {
        ...
        for (const auto& aresta : lista_de_arestas) {
            ...
            lista_adj[u].emplace_back(v, w);
        }
    }

    float get_capacidade(int u, int v) const {
        for (const auto& [vertice, peso] : lista_adj[u]) {
            if (vertice == v) {
                return peso;
            }
        }
    ...
    std::vector<std::pair<int, int>> bfs(const std::vector<std::vector<float>>& fluxos, int origem, int destino) {
        ...
        while (!fila.empty()) {
            ...
            for (const auto& [vizinho, _] : lista_adj[vertice_atual]) {
                float capacidade_residual = get_capacidade(vertice_atual, vizinho) - fluxos[vertice_atual][vizinho];
...
int main() {
    ...
    for (long unsigned int vertice = 2; vertice < grafo.lista_adj.size(); ++vertice) {
